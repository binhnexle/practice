...
export class AssetsService {
   ...
   private async asyncAssets(): Promise<any> {
      ...
      try {
         ...
         const onceADayAgoTimestamp = Math.floor(Date.now() / 1000) - 24 * 60 * 60 * 1000;
         ...
         if (Array.isArray(originalAssets?.data) && originalAssets?.data?.length > 0) {
            for (const asset of originalAssets?.data) {
               ...
               if (
                  ...
                  asset.updated_at >= onceADayAgoTimestamp &&
         ...
         this.writeData({
            ...this.data,
            assets: [
               ...this.data.assets.filter((a) => !deletedAssetIds?.includes(a.id)),
               ...syncedAssets,
            ],
         });
